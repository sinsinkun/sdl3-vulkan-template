...
ObjectPipeline::ObjectPipeline(SDL_GPUTextureFormat targetFormat, SDL_GPUDevice *gpu) {
  ...
  SDL_GPUShader *fragShader = App::loadShader(device, "obj.frag", 0, 1, 0, 0);
...
void ObjectPipeline::addTextureToObject(int id, SDL_GPUTextureFormat txFormat, Uint32 w, Uint32 h, bool isRenderTarget) {
  ...
  if (robjs.at(id).texture != NULL) {
    SDL_Log("ERR: Texture already exists for %d", id);
    return;
  }
  ...
  robjs.at(id).sampler = SDL_CreateGPUSampler(device, new SDL_GPUSamplerCreateInfo {
...
glm::mat4x4 modelMatrix(RenderObject &obj) {
...
glm::mat4x4 viewMatrix(RenderCamera &cam) {
  return glm::lookAt(cam.pos, cam.lookAt, cam.up);
}

glm::mat4x4 projMatrix(RenderCamera &cam) {
  if (cam.perspective) {
...
void ObjectPipeline::render(
  ...
  for (int i=0; i < robjs.size(); i++) {
    if (!robjs[i].visible) continue;
    if (robjs[i].vertexBuffer == NULL) {
      SDL_Log("ERR: Missing vertex data for object %d", i);
      continue;
    }
    SDL_BindGPUVertexBuffers(pass, 0, new SDL_GPUBufferBinding {
      .buffer = robjs[i].vertexBuffer,
      .offset = 0,
    }, 1);
    // build matrices
    glm::mat4x4 matrices[3] = { modelMatrix(robjs[i]), view, proj };
    SDL_PushGPUVertexUniformData(cmdBuf, 0, &matrices, sizeof(matrices));
    // upload texture if exist
    if (robjs[i].texture != NULL && robjs[i].sampler != NULL) {
      SDL_BindGPUFragmentSamplers(pass, 0, new SDL_GPUTextureSamplerBinding {
        .texture = robjs[i].texture,
        .sampler = robjs[i].sampler
      }, 1);
    }
    // draw
    if (robjs[i].indexCount > 0) {
      SDL_BindGPUIndexBuffer(pass, new SDL_GPUBufferBinding {
        .buffer = robjs[i].indexBuffer,
        .offset = 0,
      }, SDL_GPU_INDEXELEMENTSIZE_16BIT);
      SDL_DrawGPUIndexedPrimitives(pass, robjs[i].indexCount, 1, 0, 0, 0);
    } else {
      SDL_DrawGPUPrimitives(pass, robjs[i].vertexCount, 1, 0, 0);
    }
  }
}
