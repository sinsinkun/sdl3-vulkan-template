...
ObjScene::ObjScene(SDL_GPUDevice *gpu, SDL_GPUTextureFormat targetFormat) : Scene() {
  objPipe = new ObjectPipeline(targetFormat, gpu, PT_Tri, SDL_GPU_CULLMODE_BACK);
  ...
  obj1.albedo = CYAN;
  int obj2id = objPipe->uploadObject(cube(10.0f, 12.0f, 15.0f));
...
SDL_AppResult ObjScene::render(SDL_GPUCommandBuffer *cmdBuf, SDL_GPURenderPass *pass, SDL_GPUTexture* screen) {
  objPipe->render(cmdBuf, pass, screen, LightMaterial {
